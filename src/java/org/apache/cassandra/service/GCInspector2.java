...
public class GCInspector2 implements NotificationListener, GCInspectorMXBean
{
    ...
    private MemtablePool.SubPool memtable;
    private long memsize;
    private float alpha = 1.0f;
    private float goal = 0.8f;
    ...
    public GCInspector2(MemtablePool.SubPool memtable) throws Exception
    {
        ...
        this.memsize = memtable.getLimit();
        this.heapSize = Runtime.getRuntime().maxMemory();
        pw = new PrintWriter(new FileOutputStream(new File("d_memory.log"),true));
        MemMonitor2 mm = new MemMonitor2();
        Thread thd = new Thread(mm);
        thd.start();
    ...
    public static void register(MemtablePool.SubPool memtable) throws Exception
    {
        GCInspector2 inspector = new GCInspector2(memtable);
    ...
    public void handleNotification(final Notification notification, final Object handback)
    {
        ...
        if (type.equals(GarbageCollectionNotificationInfo.GARBAGE_COLLECTION_NOTIFICATION))
        {
            ...
            if (aaa < bbb) {
                if (aaa - memtable.used() > 0) {
                    memsize = (long) (alpha * (goal * heapSize - (aaa - memtable.used())));
                    memtable.setLimit((long)(memsize));
                }
                
                pw.println("used:,"+ System.nanoTime()+","+(aaa/1048576)+","+(memsize/1048576));
                pw.flush();
            }
